document.addEventListener("DOMContentLoaded", function() {
    // 1. 공통 CSS 스타일 동적 삽입 (드롭다운 호버 애니메이션 및 레이아웃 처리)
    const styleHTML = `
        <style>
            body { margin: 0; padding-top: 55px; /* 헤더 높이만큼 본문 내림 */ }
            ...
            .easytools-dropdown-content { display: none; position: absolute; top: 55px; left: 50%; transform: translateX(-50%); background-color: #ffffff; min-width: 200px; box-shadow: 0px 8px 16px 0px rgba(0,0,0,0.1); border: 1px solid #eee; border-radius: 8px; overflow: hidden; z-index: 10000; line-height: 1.5; }
            .easytools-dropdown-content a { color: #555; padding: 12px 16px; text-decoration: none; display: block; font-size: 14px; text-align: center; border-bottom: 1px solid #f9f9f9; transition: background 0.2s; }
            ...
            .easytools-footer { background: #f9f9f9; padding: 24px 20px; text-align: center; font-size: 13px; color: #888; border-top: 1px solid #eeeeee; line-height: 1.6; font-family: 'Pretendard', sans-serif; }
            .easytools-footer a { color: #888; text-decoration: none; }
            .easytools-footer a:hover { text-decoration: underline; }
    ...
    const footerHTML = `
        <footer class="easytools-footer">
            <p><strong>Easy Tools</strong> | 일상 속 복잡한 계산을 가장 심플하게 해결합니다.</p>
            <p>본 서비스의 계산 결과는 모의 계산용 참고 자료이며, 법적 효력이 없음을 안내드립니다.</p>
